...
function AlterTable() {
  ...
  const handleFetchStructure = async (clickedTableName) => {
    ...
    try {
      ...
      const response = await axios.get(`${apiURL}/get_table_structure/${clickedTableName}`);
      if (response.headers['content-type'].includes('application/json')) {
        setTableColumns(response.data.columns);
      } else {
        console.error('Unexpected response format:', response);
  ...
  const handleDropColumn = async (columnName) => {
    try {
      await axios.patch(`${apiURL}/get_table_structure/${tableName}/`, { column_name: columnName });
  ...
  const handleAddColumn = async () => {
    try {
      ...
      const response = await axios.put(`${apiURL}/get_table_structure/` + tableName + '/', { column_name: 'new_column1', data_type: 'VARCHAR(250)' });
